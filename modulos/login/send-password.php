    ...
    if (isset($_POST["email"]) && (!empty($_POST["email"]))) {
        ...
        if ($row != 0) {
            ...
            if ($reset == 0) {
                ...
                Aquí encontrara el link para hacer el cambio: </p><br>';
                $message .='<p><a href="http://smoothoperators.com.mx/modulos/login/create-new-password.php?key='.$key.'&email='.$email.'&action=reset" target="_blank">
                 http://www.smoothoperators.com.mx/modulos/login/create-new-password.php?key='.$key.'&email='.$email.'&action=reset</a></p>';
                $body = $message;
                ...
                if (!$mail->Send()) {
                   ...
                   //print_r("Message could not be sent 1. Mailer Error: {$mail->ErrorInfo}");
                }
                else {
